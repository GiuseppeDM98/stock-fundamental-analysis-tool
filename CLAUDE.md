...
**Last Updated**: July 13, 2026 — **Deep Value Rigor v2, complete** (`docs/deep-value-rigor-v2-spec.md`). Grounded mode gained a deterministic **basis-reconciliation layer** (`lib/grounding/basis.ts`: `kE`/`kB` ratios reconcile the pasted historical-multiples table against the income statement — provider vs. statement accounting space — before any multiple is applied, degrading honestly to "unavailable" rather than assuming same-basis), **11 deterministic gates** (`lib/grounding/postcheck.ts`: basis same-ness, horizon consistency, bear-breaks-price, market anchoring, trailing/forward normalization, net-debt trajectory, ROIC vs WACC, scenario probabilities → expected value, a mandatory second-method cross-check + that cross-check's own basis check), and **blind-first analyst lenses**: each lens now commits to its own bull/base/bear *before* seeing the report, then reconciles against it in a second turn where a revision requires citing a new fact — the drift between the two commitments renders in `<AnalystBlindCard>`. The whole mechanism was built to cure a real misvaluation (Iren S.p.A.: a historical multiple applied to an EBITDA on a ~17%-different basis, missed by all three second-opinion lenses) and validated end-to-end on a second live run (Webuild/WBD.MI, TIKR paste): all three lenses independently converged on the same two gate-flagged weaknesses while two of three held 0% drift on scenarios with no new fact to justify a change — direct evidence the blind commitment isn't anchoring to the report.
