...
2. **TIKR grounding + deterministic multiple anchor** (full spec in `docs/deep-value-tikr-grounding.md`): optional "Grounded" mode where the user pastes TIKR statements/valuation/peers, plus a historical-multiple anchor computed in code — the structural cure for the price-anchoring pathology. Also: deterministic quant cards (Piotroski / Altman Z / multiples percentile).
