...
**Last Updated**: July 9, 2026 — **Analyst panel: the single red-team reviewer became a panel of three independent analysts, consensus is now N-way.** Previously a saved analysis had exactly two valuations (base + one skeptic "Analyst Review") averaged into `(base + reviewer) / 2`. Now, on the **saved-analysis detail page only** (`/analyses/[id]` → `components/analyst-panel.tsx`), the user runs up to three independent second opinions through distinct lenses — **Skeptic** (the legacy red-team, keeps the `review*` columns), **Optimist/Bull**, **Quality/durability** — each streaming from `/api/ai/deep-value/verify` with an `angle` param (one route, persona/focus swapped by `buildAnalystSystemPrompt`). Each commits to its own bull/base/bear valuation, persisted in new per-analyst columns (`optimist*`/`quality*`, migration `add_analyst_panel_columns`). Consensus is now the mean of the base analysis **+ every analyst that has run**, centralized in the pure `lib/report/consensus.ts` (`consensusTriple`/`meanTriple`/`presentAnalysts`/`ANALYST_COLUMNS`) — it replaced the `(a+b)/2` averages that were copy-pasted across the analyses list, watchlist client, digest cron and evolution diff. The valuation ruler + `ComparisonTable` (`components/report/valuation-ruler.tsx`) render one colored tick/column per lens plus the consensus diamond. The **live `/analyze` panel no longer runs any review** (its review button/state were removed) — reviews attach only to a saved analysis. `AnalystAngle`/`ANALYST_ANGLES` are canonical in `types/analysis.ts`.
