...
**Last Updated**: August 24, 2026 — **DeepSeek catalog refresh**. Verified against DeepSeek's official API docs and added `deepseek-v4-flash` (a cheaper/faster tier) alongside `deepseek-v4-pro`; both now expose a genuinely distinct `low` reasoning-effort level (`low→low, medium/high/xhigh→high, max→max`, confirmed against the 2026-08-13 V4-Pro GA update — previously only `high`/`max` were offered since `low` used to collapse into `high`). Also fixed a real bug in the peak/off-peak pricing indicator (`lib/ai/deepseek-pricing.ts`): DeepSeek's peak-pricing windows are Monday–Friday only, but the UI badge was flagging peak pricing on weekends too. DeepSeek's own web search remains client-executed via Tavily — DeepSeek did add a native server-side `web_search` tool, but only on its newer Responses API (a different SDK/endpoint), not on the Anthropic-compatible endpoint this app uses.
