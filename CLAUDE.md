...
**Last Updated**: July 6, 2026 — **Analyst Review persistence + Deep Value prompt hardening**: (1) **Savable Analyst Review** — new nullable `Analysis.reviewMd` column (migration `20260706120838_add_analysis_review_md`, applied to Turso), `PATCH /api/analyses/[id]` to attach one, and a `SavedAnalystReview` component that renders a stored review or generates+persists one on the saved-analysis detail page (so running it before or after saving both work). Visually differentiated (violet box + "✓ Reviewed" pill in the analyses list). Included in the PDF export when present. (2) **Review truncation fix** — `/api/ai/deep-value/verify` was at `max_tokens: 16000`; with `xhigh` thinking + web search a real critique exhausted it and cut off mid-word. Raised to `64000`, now tracks `stop_reason` and appends a visible truncation marker (mirrors Advisor); a **"Re-run"** button regenerates. (3) **`/analyze` PDF fix** — page chrome (header/disclaimer/search/price/MoS) now `print:hidden`, so the export starts clean at the report masthead like the saved page. (4) **Buy-target bar label fix** — the saved-analyses yellow bar said "Above/Under FV" but represents the buy target → now "Above/Under Buy Target" (i18n). (5) **Prompt hardening** — Deep Value + Review Position share an `ANALYTICAL_RIGOR_BLOCK` (latest quarter not just annual, current guidance only, guidance-vs-estimate, normalized EBITDA, fundamentals-differentiated scenarios, central base case, closest comparables/structural discount, narrative↔formula consistency); the Analyst Review now treats the app's live price as authoritative (verify route calls `getQuote`) instead of "correcting" it with stale web quotes. _A future "paste financial data" grounding idea and a deterministic ticker-liveness check remain deferred — see memory `project_paste_grounding_idea`._
...
- **Analyst Review (red-team pass)**: after the report completes, a **"Run Analyst Review"** button (violet section under the report) streams an independent second-opinion critique from a fresh Opus 4.8 context via `/api/ai/deep-value/verify` (`max_tokens: 64000`, tracks `stop_reason` + appends a truncation marker like the Advisor). It stress-tests numbers/assumptions, spot-checks figures via web search, and gives a verdict on whether the base fair value holds — it does NOT rewrite the report (plain Markdown, no JSON). The verify route fetches the live price via `getQuote` and injects it as **authoritative** so the reviewer doesn't "correct" a valid price with stale web quotes. Builders `buildVerificationSystemPrompt` / `buildVerificationUserPrompt` in `lib/ai/deep-value-prompts.ts`. Client state (`reviewText`, `reviewStatus`, `reviewAbortRef`) in `deep-value-panel.tsx`, reset on each `handleGenerate()`; rendered via the shared `<ReportBody>`. A **"Re-run"** button regenerates it in the `done` state.
- **Savable Analyst Review**: the review is persisted in the nullable `Analysis.reviewMd` column. `handleSave()` attaches it when it has already run; otherwise `components/saved-analyst-review.tsx` on the detail page can run it (streaming from the verify route) and persist it via `PATCH /api/analyses/[id]` + `updateAnalysisReview()` — so review order (before/after save) doesn't matter. The saved review shows in a violet box (with a "✓ Reviewed" pill on the analyses-list card) and is included in the PDF export when present (the review box is `print:hidden` only when empty; `ReportBody` already carries `print:text-slate-900`).
- **Prompt rigor**: `buildDeepValueSystemPrompt` and `buildReviewPositionSystemPrompt` share `ANALYTICAL_RIGOR_BLOCK` (one constant in `lib/ai/deep-value-prompts.ts`, injected between the scenario and output steps) — 8 mandatory checks derived from real Analyst-Review findings: latest quarter (not just annual), current guidance only, guidance-vs-estimate labeling, normalized/recurring EBITDA, fundamentals-differentiated scenarios, central base case, closest comparables + structural-discount test, narrative↔formula consistency. Edit the constant, not the individual builders.
...
- **Analysis snapshot**: each saved report stores `priceAtAnalysis`, `fairValueBull`, `fairValueBase`, `fairValueBear`, `valuationMethod`, and `reviewMd` (the Analyst Review) — all nullable for backward compat
- **Analyses page** (`components/analyses-list.tsx`) groups analyses by ticker — each ticker is a card showing:
  ...
  - `PriceVsFVBar`: when MoS > 0, renders two stacked gradient bars — violet (intrinsic) with price label, yellow (buy target) below. Each bar has its own bear–bull range. When MoS = 0: single bar, unchanged. The yellow bar's status badge reads "Above/Under **Buy Target**" (i18n `aboveBuyTarget`/`underBuyTarget`); the MoS=0 single bar reads "Above/Under FV" (`aboveFv`/`underFv`). Labels thread `AnalysesList → TickerGroup → PriceVsFVBar`.
  - A violet **"✓ Reviewed"** pill (`analystReviewBadge`) appears on the ticker header when the latest analysis has a saved `reviewMd`.
