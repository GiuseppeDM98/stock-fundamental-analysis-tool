...
**Last Updated**: July 8, 2026 — **Analyses always position-blind + deterministic estimate-evolution diff**: The Deep Value analysis is now *always* independent of the user's position — the entire "Review Position (AI)" path was **removed** (contaminating the valuation with WAC/prevFv risked motivated-reasoning on a money decision). Gone: `buildReviewPosition{System,User}Prompt` + the `ReviewContext` type (`lib/ai/deep-value-prompts.ts`), the `reviewContext` field from the `/api/ai/deep-value` Zod schema + prompt branch, the `exitReviewContext` prop / amber "Review Position" button / WAC banner in `deep-value-panel.tsx`, the `exitReview`/`wac`/`prevFv` URL params in `analyze-client.tsx`, and the `reviewPositionBtn*` i18n keys. The Portfolio "⚠ At Fair Value" exit badge stays a **monitor** but now re-analyzes cleanly (`/analyze?ticker=X`, no position params). The value that path used to give is delivered two safe ways instead: (a) **hold/add/exit reasoning → the existing Advisor** (reads position + analyses read-only, never touches a valuation); (b) a new **deterministic estimate-evolution diff** — `computeEvolution(prev, curr)` in `lib/report/evolution.ts` + an `EvolutionDiff` block on the expanded `/analyses` card showing Δ base/reviewer/consensus vs the previous saved analysis (pure arithmetic on the intrinsic scale, never fed to a prompt). The Analyst Review (`/verify`) is unchanged (already blind; no position content to strip).
...
- `components/analyze-client.tsx`, rendered by `app/analyze/page.tsx`. Reads only `?ticker=` on mount (then `replaceState` clears it). The analysis is **always position-blind** — no WAC/prevFv is ever accepted from the URL or passed to the panel. `DeepValuePanel` is no longer gated on fundamentals/valuation — it renders as soon as the quote resolves.
...
- **Analyst Review (red-team pass)**: after the report completes, a **"Run Analyst Review"** button (violet section under the report) streams an independent second opinion from a fresh Opus 4.8 context via `/api/ai/deep-value/verify` (`max_tokens: 64000`, tracks `stop_reason` + truncation marker like the Advisor). It stress-tests numbers/assumptions, spot-checks figures via web search, gives a verdict, **and commits to its own bull/base/bear valuation** — a leading JSON block (same MoS-adjusted schema as Deep Value; the route takes `mosPercent` and buffers pre-JSON text). The critique Markdown follows; the JSON is stripped before render via `<ReportBody>`. The verify route fetches the live price via `getQuote` and injects it as **authoritative** so the reviewer doesn't "correct" a valid price with stale web quotes. Builders in `lib/ai/deep-value-prompts.ts`. Client state (`reviewText`, `reviewStatus`, `reviewResult`, `reviewAbortRef`) in `deep-value-panel.tsx`, reset on each `handleGenerate()`; `reviewResult = parseDeepValueJson(reviewText)` after the stream. A **"Re-run"** button regenerates it.
- **Savable Analyst Review**: persisted in `Analysis.reviewMd` (critique, JSON-stripped) plus the reviewer's own numbers in `reviewFairValue{Bull,Base,Bear}` + `reviewValuationMethod`. `handleSave()` attaches all of these when the review already ran; otherwise `components/saved-analyst-review.tsx` on the detail page runs it fresh and persists via `PATCH /api/analyses/[id]` + `updateAnalysisReview(id, reviewMd, reviewFvs?)` — so review order (before/after save) doesn't matter. Any new persisted field must be mirrored across all contract points (Zod POST+PATCH, GET `select`, `types/analysis.ts`, `updateAnalysisReview`). Included in the PDF export when present.
- **Prompt rigor**: `buildDeepValueSystemPrompt` injects `ANALYTICAL_RIGOR_BLOCK` (one constant in `lib/ai/deep-value-prompts.ts`, between the scenario and output steps) — 8 mandatory checks derived from real Analyst-Review findings: latest quarter (not just annual), current guidance only, guidance-vs-estimate labeling, normalized/recurring EBITDA, fundamentals-differentiated scenarios, central base case, closest comparables + structural-discount test, narrative↔formula consistency.
...
- **Report design + PDF export**: the live panel (once `status === "done"`) and the saved-analysis detail page both render `<ReportShell>` (`components/report/*`) — a masthead (company/ticker/report date), method/sector badges, the fair-value cards, the Markdown body, the recap table, and a static disclaimer footer, styled with a custom `@tailwindcss/typography` variant (`prose-report`) for an equity-research look. A **"Download PDF"** button on both surfaces calls `window.print()`; `app/print.css` hides app chrome and inverts the report to a print-safe light theme while keeping upside/downside and violet accent colors. On `/analyze` the page chrome (header, disclaimer, ticker search, price summary, MoS slider in `analyze-client.tsx`) is `print:hidden` so the PDF starts at the report masthead, not an app screenshot.
- **Analysis is always position-blind (invariant)**: no portfolio position or prior estimate is ever injected into the Deep Value or Analyst Review prompts — the valuation stays independent so its numbers can't be anchored to what the user paid or to a previous run. The former position-aware "Review Position (AI)" path was removed for this reason. Position-aware **hold/add/exit** reasoning is done in the Advisor (reads position + analyses read-only); **estimate evolution over time** is shown as a deterministic diff on `/analyses` (see Saved Analyses). Never re-introduce a position/prevFv field into `/api/ai/deep-value`.
...
- **Analysis snapshot**: each saved report stores `priceAtAnalysis`, `fairValueBull/Base/Bear`, `valuationMethod`, `reviewMd`, and the reviewer's `reviewFairValue{Bull,Base,Bear}` + `reviewValuationMethod` — all nullable for backward compat. `fairValue*` and `reviewFairValue*` are **MoS-adjusted buy targets**; reconstruct intrinsic as `stored / (1 - mos/100)`.
- **Analyses page** (`components/analyses-list.tsx`) groups analyses by ticker. Each ticker is a **compact-by-default card** (ticker · company · method · "✓ Reviewed" pill · live price · **BUY/WATCH/OVER-FV verdict** · thin `ValuationRuler compact`) that expands to:
  - a single **`ValuationRuler`** — one bear→bull axis (intrinsic scale) with emerald/amber/neutral **buy / watch / rich zones** (`getVerdict(price, buyTargetBase, intrinsicBase)`), a price dot, analysis-FV + reviewer ticks, a consensus diamond, and a **legend with values**;
  - a compact **`ComparisonTable`** (rows Bull/Base/Bear × columns Analysis / Reviewer / Consensus; consensus = mean per scenario);
  - a shared **Fair value ↔ Buy target toggle** (`level` state in `TickerGroup`) that drives BOTH ruler and table — `ComparisonTable` is controlled by `level`;
  - an **estimate-evolution diff** (`EvolutionDiff`, shown when ≥2 saves exist): `computeEvolution(analyses[1], analyses[0])` from `lib/report/evolution.ts` renders Δ base / reviewer / consensus vs the previous saved analysis on the **intrinsic** scale (grossed up per each save's own MoS), base scenario only. Pure arithmetic — reviewer/consensus rows appear only when both saves carry reviewer FVs. **Never fed to an AI prompt** (protects valuation independence);
  - metadata (performance `$priceAtSave → $priceNow ±%`, open-position WAC + live P&L) and actions (Open report, Re-analyze, Delete);
  - collapsible history of older saves for the same ticker.
  - Verdict/zone colors use static maps (`VERDICT_BADGE` / `VERDICT_TEXT`) for Tailwind purge-safety.
- Controls bar: text search, "Under FV" filter toggle, sort (recent / ticker A-Z / performance); summary count `X ticker · Y analisi`.
- **Open position banner** in the analyses *detail* page: server-side `db.position.findMany`; `OpenPositionBanner` fetches live price on mount and shows P&L.
...
- **Exit signal ("At Fair Value")**: `getExitSignal(currentPrice, analyses)` pure function (module-level) checks whether `currentPrice >= intrinsicBase` (reconstructed as `fairValueBase / (1 - mosPercent/100)`) of the most recent saved analysis. When triggered: (1) aggregated row shows an amber `⚠ At Fair Value` pill badge + "Re-analyze →" button always visible in the price/P&L row; (2) inside the "saved analyses" collapse, an amber monitor banner shows the intrinsic value inline. It is a **monitor only** — both buttons navigate to a **clean** `/analyze?ticker=X` via `window.location.href` (no position params; the re-analysis is position-blind by design). Hold/exit reasoning is then done in the Advisor.
...
lib/
  ai/deep-value-prompts.ts # Prompt builders for deep value + Analyst Review (verify) — always position-blind
...
lib/report/evolution.ts    # computeEvolution() — deterministic estimate diff vs previous save (no AI)
...
docs/                  # Feature specs
__tests__/             # yahoo-client.test.ts + evolution.test.ts (computeEvolution)
```
