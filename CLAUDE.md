...
**Last Updated**: July 10, 2026 — **Buy-target verdict is now consensus-aware.** On `/analyses`, `/watchlist` and the email digest, the Buy/Watch/Over badge, the ruler's zone boundaries, and the buy-target percentage now prefer the analyst-panel **consensus** (base analysis + every lens that ran) over the base analysis alone whenever at least one lens has run, falling back to the base analysis otherwise. The percentage phrase itself now names its source — "N% above/below buy target (analysis)" vs "... (consensus)" — fixing an ambiguity where the base analysis's own buy target and the consensus figure can sit close but differ (e.g. 13.96 vs 13.94) with no way to tell which one a bare percentage referred to. See AGENTS.md → "Shared valuation ruler module" for the pattern (zone-boundary props and `getVerdict`'s inputs must move together, never independently) and → "Watchlist card + ruler" for the translation-key/email mirroring.
