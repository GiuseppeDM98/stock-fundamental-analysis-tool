...
- **Bug PRE-ESISTENTE → FIXATO in questo branch**: `sfa:lastTicker` — scriveva la stringa grezza ma `getStorageItem` fa `JSON.parse` → throw → fallback sempre ad `AAPL` (persistenza ultimo ticker rotta da prima del refactor). Corretto scrivendo `JSON.stringify(ticker)` in `analyze-client.tsx`.
