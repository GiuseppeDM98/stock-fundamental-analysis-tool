...
`buildDeepValueSystemPrompt` injects `ANALYTICAL_RIGOR_BLOCK` (a module-level const in `lib/ai/deep-value-prompts.ts`, interpolated between the scenario step and the output step). It carries **12 mandatory rigor checks** distilled from real reviewer findings on live reports (latest quarter not just annual; current dated guidance only; guidance-vs-estimate labeling; normalized/recurring EBITDA; fundamentals-differentiated scenarios reconciled to the stated sensitivity; central base case **re-derived at the base driver, not a stale TTM**; closest-comparables on the **same basis** + anti-selection-bias + structural-discount test; **complete EV→equity bridge with minorities, varied per scenario**; method↔narrative coherence (no disguised SOTP); **anchor the valuation lever to something independent of the price — the market-implied read is a control that reports the GAP, never the input**; MoS-adequacy commentary; internal-consistency + reconciliation linter (ROE/margin/equity/share-count/terminology)). Checks are written **conditionally** (they bite only where relevant — e.g. commodity-normalization for cyclicals, the bridge for firms with minorities, the multiple anchor for multiple methods), so they are no-ops on a clean non-cyclical.
...
Optional mode on `/analyze`: the user pastes typed blocks (income statement, balance sheet, cash flow, historical multiples, forward estimates, peer comps) instead of relying solely on the AI's own web search. `lib/grounding/*` (pure, no `server-only`) — `merge.ts`/`anchors.ts`/`reconcile.ts`/`postcheck.ts`/`prompt-format.ts`/`schema.ts` — computes historical multiple stats, a 3×3 valuation grid, the market-implied multiple (a **control**, never an input — rigor item 10 above), and reconciliation warnings. Full design: `docs/deep-value-grounding-spec.md` (implemented; validated end-to-end on a live run, including the analyst-panel lenses picking up a real 2% bridge deviation via the same deterministic grid the base report saw).

- **Injection split — system carries rules, user carries data.** `buildDeepValueSystemPrompt`/`buildAnalystSystemPrompt` take `grounding` only as a presence flag, to gate the static `GROUNDED_RULES_BLOCK` (web search rescoped not eliminated, the Estimates trap, units/currency, anchors-are-facts). `buildDeepValueUserPrompt`/`buildAnalystUserPrompt` render the actual numbers via `formatGroundingForPrompt()`. Mirrors the instructions-vs-facts split the two builders already had. Quick mode (`grounding` omitted) must stay byte-identical — re-verify this FIRST whenever touching these builders, via a throwaway capture-and-diff test (params captured before the edit, compared after), not a maintained golden fixture.
