...
- **Catalog**: `types/ai-settings.ts` — `AI_MODEL_CATALOG` (`claude-opus-4-8` / `claude-sonnet-5` / `deepseek-v4-pro` / `deepseek-v4-flash`), each with `provider`, `efforts` (subset of `AI_EFFORT_LEVELS`), `supportsWebSearch`. Both DeepSeek models expose `low`/`high`/`max` — as of the 2026-08-13 V4-Pro GA update, DeepSeek maps effort as low→low, medium→high, high→high, xhigh→high, max→max (`low` is now a genuinely distinct level; `medium`/`xhigh` are omitted from the catalog since they're indistinguishable from `high`; verified against `https://api-docs.deepseek.com/guides/thinking_mode/` 2026-08-24). `DEFAULT_AI_SETTINGS` = Opus 4.8 / high / thinking on.
- **DeepSeek peak/off-peak pricing** (confirmed 2026-08-24 against `https://api-docs.deepseek.com/quick_start/pricing/`): off-peak rates are 50% of peak, in effect since 2026-08-16. Peak windows are fixed UTC `01:00–04:00` and `06:00–10:00`, **Monday–Friday only** (weekends are always off-peak). `lib/ai/deepseek-pricing.ts` (`isDeepSeekPeakHour()` / `formatDeepSeekPeakWindows()`, pure, no `server-only`) computes this and converts to a display timezone (DST-aware); `<AiSettingsControl>` shows a live "tariffa doppia ora"/"tariffa normale" badge for DeepSeek models, re-checked every 60s. DeepSeek's own `web_search` did gain a genuine server-side implementation, but only on their newer Responses API (OpenAI SDK/format, different endpoint from `https://api.deepseek.com/anthropic`) — not on the Anthropic-compat endpoint this app uses, so the Tavily-based client-executed workaround (gotcha #25 below) still applies.
...
28. **`runCreateWithToolLoop` (the non-streaming tool loop) has no `hitIterationCap` signal like its streaming sibling `runStreamWithToolLoop`.** If DeepSeek is still mid-`web_search` when `maxIterations` runs out, the loop just returns the last message — which is itself another `tool_use` call, not a final answer — so the caller's own JSON-block regex finds nothing. In `/api/earnings` this surfaced as a generic `502 "AI returned no parseable result"`, indistinguishable from a genuinely malformed response. Reproduced in production (2026-08-24): the route was left on the loop's 10-round default (sized for Claude, whose server-side search never surfaces a client `tool_use` here), which is too tight for DeepSeek's client-executed one-query-at-a-time pattern on some tickers. Fixed by moving it onto the shared `DEEP_RESEARCH_MAX_TOOL_ITERATIONS` (see the Shared tool loop bullet above) — but the underlying silent-failure shape isn't fixed, only given more room before it can recur.
29. **A single malformed field anywhere in the model's leading JSON block discards the ENTIRE valuation.** `parseDeepValueJson`/`parseValuationMeta` both do a bare `JSON.parse()` on the captured ` ```json ` block with no partial-recovery path — if the model drops a closing brace on a nested field (reproduced in production, 2026-08-24: `crossCheck` after an unusually long free-text `reconciliation` string), the whole object fails to parse even though `method`/`sector`/`bull`/`base`/`bear` were generated correctly and sit right there in the text. Symptom on a saved analysis: the list card reads "no valuation data" (the client-side parse already failed at save time, so `fairValueBull/Base/Bear` were never persisted) AND the detail page falls back to its raw-markdown branch. That fallback (`app/analyses/[id]/page.tsx`) used to render `{markdown}` directly inside a `<div>` instead of through `<ReportBody>` — fixed, so the report text stays readable now — but no resilience was added to the JSON parse itself; a single bad nested field still loses the whole summary. A targeted partial-field extraction (regex the 5 required top-level fields independently of the optional `assumptions`/`crossCheck` block) would fix this class of failure but wasn't implemented — noted as a known gap, not yet worth the complexity for what appears to be a rare model glitch.
