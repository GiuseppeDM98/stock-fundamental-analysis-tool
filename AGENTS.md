...
**Key difference from `/analyses`: the watchlist buy target uses the MoS of the *watchlist item*, not the MoS stored on the analysis.** The analysis still drives the verdict/Δ% (it's the source of the intrinsic value); the watchlist item's own `mosPercent` slider re-derives the buy target from that intrinsic value. Analyst-lens/consensus values are markers only, same as `/analyses`.
