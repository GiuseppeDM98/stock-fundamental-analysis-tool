...
- **Analyst Panel** — on a saved analysis, run up to three independent second-opinion passes through distinct lenses: **Skeptic** (adversarial by design — builds a **kill price**, the price below which the thesis is dead, or says explicitly it can't), **Optimist** (constructive bull case), and **Quality** (moat/durability). No lens opens with praise; each spot-checks figures via web search and **proposes its own Bull/Base/Bear valuation**. On a Grounded analysis, each lens first commits to its valuation **before** it ever sees your report, then reconciles against it — a "blind vs. final" card shows the drift between the two, so you can see whether the opinion is genuinely independent. Results are saved with the analysis, shown in a distinct panel with a **✓ Reviewed** badge, and included in the PDF export. On the saved-analyses card every lens that ran sits beside the base analysis with an N-way **consensus** (average of the base + every lens run) per scenario
- **User accounts** with saved reports — revisit your analyses anytime
- **Portfolio tracker** — track real purchases with live P&L, multi-currency FX conversion, DCA aggregation (Weighted Average Cost per ticker), and cross-links to saved analyses per ticker. A **"+ Purchase"** button on each holding pre-fills the ticker/name/currency for a quick additional buy on the same stock.
...
- **Close a position** — sell all or part of a holding; closed positions are archived with realized P&L, and the P&L history chart marks the sale date so it's distinguishable from a market drop
- **Daily price change** — each portfolio position shows today's % move vs. the previous close, inline next to the current price
- **Capital gains tax** — set an optional tax rate per position to see estimated taxes and net P&L alongside gross gains, on both open positions and realized (closed) sales; dividend totals also show gross and estimated net amounts. The estimate only appears when the relevant total (unrealized or realized) is itself a net gain
- **Analysis performance tracking** — see how the stock price moved since you saved each analysis vs. fair value
- **Next-earnings calendar (AI-sourced)** — a per-stock button on the analyses, watchlist, and portfolio pages looks up the next results-release date via AI web search and remembers it. The analyses page lists upcoming dates nearest-first and flags an analysis as stale ("New data since last analysis") once the company has reported since you last analysed it — so you know exactly when to re-run. Handles quarterly, half-year, and annual reporters
- **Watchlist + weekday email digest** — add tickers to a personal watchlist; each ticker is a compact card that expands into the same **valuation ruler** used on the saved-analyses page (buy/watch/rich zones, your fair value, one tick per analyst lens that ran, consensus) with a Fair value ↔ Buy target toggle. A **Monday–Friday** email digest (8am local, adjusts automatically for daylight saving; no weekend emails since markets are closed) breaks down Bear/Base/Bull by Analysis / each analyst lens that ran / Consensus alongside price and gap vs. your buy target, plus a countdown to (or since) the next-earnings date when you've looked one up. If you already hold a position in a watched ticker, the digest shows your shares, average cost, and P&L instead of suggesting it as a fresh buying opportunity. Pause emails with a single toggle when you're not actively investing. Each card shows a **price proximity badge** (distance from your buy target) and an inline **Analyze** action button.
...
- **Choose your AI model** — pick which model powers each analysis (Claude Opus 4.8, Claude Sonnet 5, or **DeepSeek V4 Pro / V4 Flash**), plus a reasoning-effort level (Low/High/Max) and thinking on/off, as a global default (gear icon in the nav bar) with a per-run override on Deep Value, the Advisor, and the Analyst Panel. DeepSeek models also show a live peak/off-peak pricing indicator
...
- 📋 **Saved Analyses by Ticker**: Analyses page groups reports by stock as compact cards with a **Buy / Watch / Over-FV verdict** and live price; expand for a single **valuation ruler** (current price on the Bear→Bull range with buy/watch zones, your fair value, one tick per analyst lens that ran, and the consensus) plus a comparison table. Once an analyst lens has run, the verdict/zones/percentage weigh the **consensus** over the base analysis alone (falling back to the base analysis otherwise) — the on-screen percentage always names which one it's measured against. Search, filter (Under FV), and sort across all your saved research
- 🧮 **Valuation Metrics**: P/E, P/FCF, FCF Yield, Earnings Yield cards with YoY trend and educational tooltips
- 💼 **Portfolio Tracker**: Track real purchases with live P&L per position + DCA aggregation (Weighted Average Cost per ticker) + multi-currency aggregate summary (EUR conversion via Frankfurter) + cross-links to saved analyses per ticker. A per-ticker **"+ Purchase"** button pre-fills ticker/name/currency/ISIN for a follow-on buy
- 🔖 **Close Position**: Sell all or part of a holding — closed positions archive with realized P&L, and a violet marker on the P&L chart flags the sale date
...
- 🧾 **Capital Gains Tax**: Set an optional tax rate per position to see estimated taxes and net P&L on both unrealized and realized (closed-position) gains; dividend totals show gross and estimated net
- 📈 **Analysis Performance**: See how price moved since saving vs. fair value — "Under FV" / "Above FV" badge per report
- 📅 **Next-Earnings Calendar**: A per-stock AI button (on analyses, watchlist, and portfolio) finds the next results-release date via web search and saves it; the analyses page lists upcoming dates nearest-first and nudges you to re-analyse once a company has reported since your last analysis — quarterly, half-year, or annual reporters alike
- 🌐 **EN / IT UI**: Switch the entire interface language from the navbar; preference saved automatically
- 🔬 **Analyst Panel**: On a saved analysis, run up to three independent second-opinion lenses — Skeptic (adversarial, builds a kill price), Optimist (bull case), Quality (moat/durability) — each spot-checks figures via web search and proposes its own Bull/Base/Bear valuation (shown vs. the base analysis with an N-way consensus). On Grounded analyses, each lens commits blind before seeing the report, then shows the drift on reconciliation. Save with the analysis and export in the report PDF
- 📬 **Watchlist + Weekday AI Email Digest**: Add any ticker to your watchlist — a valuation-ruler card per ticker (same UI as saved analyses) plus a Mon–Fri email with Bear/Base/Bull broken down by Analysis/each analyst lens/Consensus, current price, upside vs. your buy target, an earnings countdown, and status badges — pause with a toggle when not actively investing. The digest is portfolio-aware: if you already hold the ticker, it shows your shares/average cost/P&L and frames an under-target price as a "top up" rather than a fresh buy
...
- 📎 **Grounded Deep Value**: Optionally paste your own financial statements, historical multiples, forward estimates, and peer comps on `/analyze` instead of relying solely on AI web search — a verifiable preview shows the transcribed data and historical multiple distribution before you run the analysis (now with a basis-reconciliation check that catches a historical multiple computed on a different EBITDA definition than your own statements), and a deterministic **post-check** dashboard recomputes the model's own valuation bridge afterward across 11 checks: bridge arithmetic, whether the bear case genuinely breaks today's price, whether bull/bear share the same horizon, dividend coverage by free cash flow, returns vs. cost of capital, and whether the mandatory second valuation method reconciles — plus a probability-weighted expected value
...
- [x] Analyst Panel — three independent second-opinion lenses (Skeptic/Optimist/Quality) with N-way consensus on a Deep Value report
