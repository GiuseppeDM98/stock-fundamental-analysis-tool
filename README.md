...
- 📬 **Watchlist + Daily AI Email Digest**: Add any ticker to your watchlist — a valuation-ruler card per ticker (same UI as saved analyses) plus a daily email with Bear/Base/Bull broken down by Analysis/each analyst lens/Consensus, current price, upside vs. your buy target, and status badges — pause with a toggle when not actively investing. The digest is portfolio-aware: if you already hold the ticker, it shows your shares/average cost/P&L and frames an under-target price as a "top up" rather than a fresh buy
