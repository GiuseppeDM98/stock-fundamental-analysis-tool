...
- **Analyst Panel** — on a saved analysis, run up to three independent second-opinion passes through distinct lenses: **Skeptic** (red-teams the numbers and assumptions), **Optimist** (constructive bull case), and **Quality** (moat/durability). Each lens spot-checks figures via web search and **proposes its own Bull/Base/Bear valuation**. Results are saved with the analysis, shown in a distinct panel with a **✓ Reviewed** badge, and included in the PDF export. On the saved-analyses card every lens that ran sits beside the base analysis with an N-way **consensus** (average of the base + every lens run) per scenario
...
- 🔬 **Analyst Panel**: On a saved analysis, run up to three independent second-opinion lenses — Skeptic (red-team), Optimist (bull case), Quality (moat/durability) — each spot-checks figures via web search and proposes its own Bull/Base/Bear valuation (shown vs. the base analysis with an N-way consensus). Save with the analysis and export in the report PDF
